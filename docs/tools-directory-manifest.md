...
**Tool count:** 67 (29 read, 38 write) · **Refresh from live server card** · Annotations vary by tool (`readOnlyHint`, `destructiveHint`)
...
Write and image tools (`create_*`, `update_*`, `delete_*`, `init_image_upload`, etc.) are not listed in this table. Refresh the full catalog from production (see below) or see [mcp-tool-reference.md](./mcp-tool-reference.md).
